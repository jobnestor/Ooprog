...
class Person  {                   //  Abstrakt baseklasse.
  ...
  public:
    ...
    Person(ifstream & inn)  {
		char temp[MAXTXT + 1];					//<fornavn> // Leser /n i getline
		inn.getline(temp, MAXTXT);			//<fodselsdato>
...
class Ansatt : public Voksen  {        //  Konkret klasse.
  ...
  public:
	...
	bool harPartner()	{	(partner) ? true : false;	}
	...
	int barnFodt(char aaa[])
	{
		if (antBarn != 0)
		{
			...
			{
				if(barn[i]->likAar(aaa)) return nr;
				else return 0;
			}
		}
		else return 0;
...
void partnerEndring()  {              //  P - ENDRE DATA OM PARTNER:
	...
	if(finnesAllerede(ansNr) != 0)	{
		if(ansatte[ansNr]->harPartner() == false)	{
